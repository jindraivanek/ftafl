...
module Model =
    let init f : Model<_> =
        ...
        let addAttr =
            addEntity attrIdGen
                (fun m k v -> { m with Attrs = Map.add k v m.Attrs })
        let addUnit =
            addEntity unitIdGen
                (fun m k v -> { m with Units = Map.add k v m.Units })
        let addBoard =
            addEntity boardIdGen
                (fun m k v -> { m with Boards = Map.add k v m.Boards })
        let addPlayer =
            addEntity playerIdGen
                (fun m k v -> { m with Players = Map.add k v m.Players })            
        let postUpdate f = m <- f m
        f addAttr addUnit addBoard addPlayer postUpdate
        m
...
let getUnitActions uId model =
    ...
    Seq.append (b.Rules.GetMoves uId model) (u.Attrs
                                             |> Map.toSeq
                                             |> Seq.collect (fun (aId, x) ->
                                                    let a = getAttr aId model
                                                    a.Rules.GetMoves uId model))
    |> Seq.filter
           (fun (_, msg) -> not (List.isEmpty <| model.ToCoreEv model msg))

let getActivePlayerActions model =
    model.Units |> Map.toSeq |> Seq.filter (fun (_,u) -> u.Owner = model.ActivePlayer)
...
let updateSingle model ev =
    ...
    | ChangeAttr(uId, AttrDelta(attr, x)) ->
        ...
        |> updateUnit uId (fun u ->
               ...
               { u with Attrs =
                            ((if newValue > 0 then Map.add attr newValue
                              else Map.remove attr) u.Attrs) })
...
let intercept<'Msg> model (msg : 'Msg) =
    ...
    let newEvs =
        ...
        ||> Seq.fold (fun evs uId ->
                ...
                let evs =
                    evs
                    |> List.collect (fun ev -> b.Rules.Intercept uId ev model)
                (evs,
                 u.Attrs
                 |> Map.toSeq
                 |> Seq.map fst)
                ||> Seq.fold (fun evs aId ->
                        ...
                        let newEvs =
                            evs
                            |> List.collect
                                   (fun ev -> a.Rules.Intercept uId ev model)
                        newEvs))
